...
def montar_estatisticas_do_ano(ano: int, caminho: Path, df: pd.DataFrame, saida: RegistradorDeSaida) -> dict:
    ...
    saida.escrever(f"  - Longitudes ausentes ou nao numericas apos conversao: {coord['lon_nao_numerica']}")
...
def gerar_resumo_executivo(resumos: list, saida: RegistradorDeSaida):
    ...
    saida.escrever("  3. O texto literal 'NA' foi preservado como categoria em")
    saida.escrever("     classificacao_acidente. Consultar o dicionario da PRF antes de")
    saida.escrever("     decidir se o valor sera mantido como 'NA', renomeado para")
    saida.escrever("     'Nao informado' ou tratado de outra forma.")
    saida.escrever("     Nenhum campo vazio foi identificado nas tres bases.")
